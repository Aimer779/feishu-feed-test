...
def main():
    config = get_delivery_config("x")
    parser = argparse.ArgumentParser(description="X 端到端测试")
    parser.add_argument(
        "--hours",
        type=int,
    ...
    print(
        f"Step 1: fetcher -> X tweets "
        f"(fetch last {hours}h, scheduled cadence {config.cadence_hours}h)"
    )
    articles = fetch_x(hours=hours)
    print(f"Fetched {len(articles)} articles")

    if len(articles) < 5:
        print(f"\nToo few articles ({len(articles)} < 5). Exiting.")
        return
    ...
    if item_total == 0:
        print("\nNo items generated by summarizer. Exiting.")
        return
